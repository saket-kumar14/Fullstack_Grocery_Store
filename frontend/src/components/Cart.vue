...
export default {
  ...
  methods: {
    fetchCartData() {
      axios.get(`https://grocery-store-kwa8.onrender.com/cart/${localStorage.getItem('userId')}`)
      .then(response => {
    ...
    deleteProduct(productId, quantity) {
      ...
      axios.get(`https://grocery-store-kwa8.onrender.com/product/${productId}`)
      .then(response => {
      ...
      axios.post(`https://grocery-store-kwa8.onrender.com/edit-product/${productId}`, updatedProduct, {
          ...
          .then(() => {
            axios.post(`https://grocery-store-kwa8.onrender.com/cart/${localStorage.getItem('userId')}/${productId}`)
            .then(response => {
    ...
    placeOrder() {
      ...
      axios.post(`https://grocery-store-kwa8.onrender.com/placeOrder/${localStorage.getItem('userId')}`, orderDetails,)
        .then(response => {
          console.log('Order placed successfully:', response.data);
          axios.post(`https://grocery-store-kwa8.onrender.com/cart/${localStorage.getItem('userId')}`)
            .then(() => {
              location.reload()
            })
            .catch(error => {
