...
export default {
  ...
  methods: {
    fetchProducts() {
      axios.get(`https://grocery-store-kwa8.onrender.com/shop_by_category?category_id=${this.categoryId}`)
        .then(response => {
    ...
    addToCart(productId,name,price,quantity) {
      ...
      axios.post('https://grocery-store-kwa8.onrender.com/cart', data,{
        headers:{
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      })
      .then(() => {
        axios.post(`https://grocery-store-kwa8.onrender.com/edit-product/${productId}`, updatedProduct, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            }
          })
    ...
    addNewProduct() {
      ...
      if (localStorage.getItem('userRole') === 'manager') {
        axios.post(`https://grocery-store-kwa8.onrender.com/add-product-request/user=${localStorage.getItem('email')}&role=${localStorage.getItem('userRole')}`, newProduct, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            }
          })
      ...
      axios.post('https://grocery-store-kwa8.onrender.com/add-product', newProduct, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      })
    ...
    editProduct(productId, name) {
      ...
      if (localStorage.getItem('userRole') === 'manager') {
        axios.post(`https://grocery-store-kwa8.onrender.com/edit-product-request/user=${localStorage.getItem('email')}&role=${localStorage.getItem('userRole')}/${productId}/${name}`, updatedProduct, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            }
          })
    ...
    deleteProduct(productId, name) {
      axios.post(`https://grocery-store-kwa8.onrender.com/delete-product/user=${localStorage.getItem('email')}&role=${localStorage.getItem('userRole')}/${productId}/${name}`)
        .then(() => {
          setTimeout(() => {
            location.reload();
          }, 300);
