...
export default {
  ...
  methods: {
    ...
    fetchRequests() {
      axios.get('https://grocery-store-kwa8.onrender.com/requests',{
        headers: {
    ...
    fetchUsers() {
      axios.get('https://grocery-store-kwa8.onrender.com/users',{
        headers: {
          Authorization: `${localStorage.getItem('accessToken')}`
        }
      })
    ...
    changeRole(userId, userRole) {
      axios.post(`https://grocery-store-kwa8.onrender.com/change-role/${userId}/${userRole}`,{
        headers:{
    ...
    deleteUser(userId) {
      axios.post(`https://grocery-store-kwa8.onrender.com/delete-user/${userId}`)
        .then(() => {
            location.reload();
    ...
    declineRequest(requestId) {
      axios.post(`https://grocery-store-kwa8.onrender.com/decline-request/${requestId}`)
        .then(() => {
          this.fetchRequests();
          setTimeout(() => {
            location.reload();
    ...
    approveRequest(requestId) {
      axios.get(`https://grocery-store-kwa8.onrender.com/get-request/${requestId}`)
      .then(response => {
    ...
    postApprovedData(requestData, requestId) {
      axios.post('https://grocery-store-kwa8.onrender.com/post-approved-data', requestData)
      .then(() => {
