...
export default {
  ...
  methods: {
    productsCSV(){
      axios.get(`https://grocery-store-kwa8.onrender.com/export-products-csv`)
      .then(response => {
        axios.get(`https://grocery-store-kwa8.onrender.com/export-products-csv/${response.data.task_id}`, { responseType: 'blob' })
        .then(response => {
    ...
    inactiveUsersAlert(){
      axios.get(`https://grocery-store-kwa8.onrender.com/alert-inactive-users`)
        .then(response => {
          axios.get(`https://grocery-store-kwa8.onrender.com/alert-inactive-users/${response.data.task_id}`)
          .then(response => {
          })
          .catch(error => {
    ...
    getUserIdRole(){
      if(localStorage.email!==''){
        axios.get(`https://grocery-store-kwa8.onrender.com/user-idrole/${localStorage.email}`)
        .then((response)=>{
    ...
    fetchCategories() {
      axios.get('https://grocery-store-kwa8.onrender.com/')
        .then(response => {
    ...
    addNewCategory() {
      if (this.newCategoryName.trim() !== '') {
        ...
        if (localStorage.getItem('userRole') === 'manager') {
          axios.post(`https://grocery-store-kwa8.onrender.com/add-category-request/user=${localStorage.getItem('email')}&role=${localStorage.getItem('userRole')}`, newCategory, {
              headers: {
        ...
        axios.post('https://grocery-store-kwa8.onrender.com/add-category', newCategory,{
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            }
          })
    ...
    updateCategory(categoryId, name){
      ...
      if (localStorage.getItem('userRole') === 'manager') {
        axios.post(`https://grocery-store-kwa8.onrender.com/edit-category-request/user=${localStorage.getItem('email')}&role=${localStorage.getItem('userRole')}/${categoryId}/${name}`, updateCategoryVals, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            }
          })
      ...
      axios.post(`https://grocery-store-kwa8.onrender.com/edit-category/${categoryId}`, updateCategoryVals,{
        headers:{
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      })
    ...
    deleteCategory(categoryId, name) {
      axios.post(`https://grocery-store-kwa8.onrender.com/delete-category/user=${localStorage.getItem('email')}&role=${localStorage.getItem('userRole')}/${categoryId}/${name}`)
        .then(() => {
          setTimeout(() => {
            location.reload();
          }, 300);
