...
export default {
  ...
  methods: {
    fetchOrdersData() {
      axios.get(`http://127.0.0.1:4000/orders/${localStorage.getItem('userId')}`)
      .then(response => {
    ...
    exportCSV() {
      axios.post('http://127.0.0.1:4000/initiate-export-orders', {
    ...
    checkTaskStatus(taskId) {
      axios.get(`http://127.0.0.1:4000/export-orders-status/${taskId}`)
        .then(response => {
